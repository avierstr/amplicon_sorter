...
def save_arguments(): # save all settings in the result.txt file
    ...
    with open(os.path.join(outputfolder,'results.txt'), 'a') as rf:
        rf.write('-----------------------------------------------------------\n')
        rf.write('amplicon_sorter single core version: ' + version + '\n')  
        rf.write('-----------------------------------------------------------\n')
...
def make_consensus(todolist, outputfolder, consensus_tempfile, infile):
    ...
    with open(os.path.join(outputfolder, consensus_tempfile), 'a') as f:
        ...
        f.flush()
#==============================================================================
def iden_consensus(todolist, outputfolder, consensus_tempfile, _):
    templist = []
    for  A1, A2, y, z in todolist:
        idenlist = []
    ...
    with open(os.path.join(outputfolder, consensus_tempfile), 'a') as f:
        ...
        f.flush()
#==============================================================================
def do_parallel(outputfolder, consensus_tempfile, make_consensus, 
    ...
    for dirpath, dirnames, filenames in os.walk(outputfolder):
        ...
        for name in filenames:
            ...
            with open(os.path.join(outputfolder, name), 'rb') as rf:
                ...
                time.sleep(2)
            os.remove(os.path.join(outputfolder, name))
            make_consensus(todolist, outputfolder, consensus_tempfile, group_filename)
#==============================================================================
def comp_consensus_groups(grouplist): # compare consensuses with each other
    ...
    while a1 > a2: # limit number of cycles 
        ...
        do_parallel(outputfolder, consensus_tempfile, make_consensus, 
                      stringx, '_')
        ...
        do_parallel(outputfolder, consensus_tempfile, iden_consensus, 
                      stringx, '_')
...
def process_consensuslist(indexes, grouplist, group_filename):  
    ...
    if len(todolist) > 0:
        stringx = '...processing: '
        do_parallel(outputfolder, group_tempfile, similarity_species, 
                      stringx, '_')
#============================================================================== 
def similarity_species(todolist, outputfolder, group_tempfile, _):
